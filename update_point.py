...
def insert_json_data(point_id, opc_tag, describe, device_description_chinese, serial, instrument, device_name_chinese, belong_co, status, value_min, value_max, unit, last_monitor_time, thickener_id, mixer_id):
    data = {
        "point_id": point_id,
        "opc_tag": opc_tag,
        "describe": describe,
...
if __name__ == '__main__':
    ...
    for rownum in range(nrows):
        ...
        if row:
            # 更新点位管理表的数据
            point_query = col.find_one({"point_id": point_id})
            if point_query is None:
                ...
                col.update_one(point_query, newvalues)
